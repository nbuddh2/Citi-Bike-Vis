...
export class RideMapBoxComponent implements OnInit {
  ...
  private loadSourceData() {
    ...
    this.map.addSource("nyc", {
      type: "geojson",
      data: "src/assets/statistics/ride.json",
      generateId: true,
    });
  }
    ...
    this.map.on("load", (event) => {
      ...
      this.mapService.neiborShowSource.subscribe((neighbor) => {
        if (neighbor != "") {
          ...
          var center_x = d3.mean(points, function (d) {
            return d[0];
          });
          var center_y = d3.mean(points, function (d) {
            return d[1];
          });

          var coordinate = mapboxgl.LngLat.convert([center_x, center_y]);
          popup.setLngLat(coordinate).setHTML(description).addTo(this.map);
        }
      });
